...
Smart Tune probes the selected media with `ffprobe`, preserves its source resolution and aspect ratio on the first attempt, and chooses a guarded H.264/AAC starting point from the current [YouTube Live encoder recommendations](https://support.google.com/youtube/answer/2853702): 4 Mbps for 720p30, 6 Mbps for 720p60, 10 Mbps for 1080p30, 12 Mbps for 1080p60, 15/24 Mbps for 1440p30/60, and 30/35 Mbps for 2160p30/60. CPU and memory select the initial preset/FPS, while the median TCP delivery rate previously observed on the Agent-owned FFmpeg socket can cap bitrate; 20% network headroom is retained. During runtime recovery, 1080p is protected by a 24 FPS and 8 Mbps quality floor; sustained encoder pressure may switch to a faster preset, then lower FPS only to that floor, and finally lower resolution to 720p. 720p is the minimum output resolution and is not reduced further. Set `STREAM_AGENT_EGRESS_CAPACITY_KBPS` to a trusted upload speed-test result when no prior socket estimate exists.

FFmpeg writes machine-readable progress every second. The watchdog treats a live process with no progress for 20 seconds (after a 15-second startup grace period) as stalled and runs it through the same verified recovery path as an exited process. Override these thresholds with `STREAM_AGENT_PROGRESS_STALL_SECONDS` and `STREAM_AGENT_PROGRESS_GRACE_SECONDS`. YouTube auto-tuning uses a five-sample runtime window, requires two matching critical checks or three matching warning checks, and keeps the configured cooldown between controlled restarts. Every 24 hours of observed runtime, the Hub records the parameter level with the longest proven stable duration for each Agent, source, and YouTube stream; that preferred level is synchronized to the Agent and used first for automatic or manual recovery. Temporary YouTube API failures use short bounded retries and do not discard a pending diagnosis; an existing Agent reuses its protected previous ingestion target for the same stream while the API recovers.
...
Smart Tune 会先用 `ffprobe` 读取源视频，并在第一次开播时优先保留源视频的原始分辨率和比例，再以 YouTube Live 当前 H.264 建议值作为初始基线：720p30/60 为 4000/6000 Kbps、1080p30/60 为 10000/12000 Kbps、1440p30/60 为 15000/24000 Kbps、2160p30/60 为 30000/35000 Kbps。CPU 和可用内存只决定初始 preset/FPS；运行中 1080P 受 24 FPS、8 Mbps 质量下限保护，持续编码压力过高时先换更快 preset，再将 FPS 最低降到该下限，最后才降到 720P。720P 是最低输出分辨率，不再继续降级；没有历史样本时可通过 `STREAM_AGENT_EGRESS_CAPACITY_KBPS` 填入可信测速结果。

FFmpeg 每秒写入结构化进度。进程存活但默认连续 20 秒无进度时，Watchdog 会按卡死处理并走统一的验证恢复流程。YouTube 自动调参使用最多五个运行样本，严重问题连续两次、警告连续三次才允许在冷却期后受控重启，减少短时告警造成的反复断流。Hub 每 24 小时按 Agent、源文件和 YouTube 直播流统计已观测到的稳定时长，把最长稳定的参数层级记录为该 VPS 的适宜配置，并同步给 Agent；自动恢复和手动重启都会优先使用它。YouTube API 短暂失效时会短间隔、有限退避重试，不会清空已确认的问题；同一直播流会由 Agent 复用受保护的上次推流地址，待 API 恢复后继续正常检查和调节。
