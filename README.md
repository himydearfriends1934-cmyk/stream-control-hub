...
FFmpeg 每秒写入结构化进度。进程存活但默认连续 20 秒无进度时，Watchdog 会按卡死处理并走统一的验证恢复流程。YouTube 自动调参使用最多五个运行样本，严重问题连续两次、警告连续三次才允许在冷却期后受控重启，减少短时告警造成的反复断流。YouTube API 短暂失效时会短间隔、有限退避重试，不会清空已确认的问题；同一直播流会由 Agent 复用受保护的上次推流地址，待 API 恢复后继续正常检查和调节。
