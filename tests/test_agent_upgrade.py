...
class AgentUpgradeTests(unittest.TestCase):
    ...
    def test_ui_remembers_last_agent_and_keeps_role_actions_in_settings(self):
        ...
        self.assertNotIn('class="left-stack"', app.HTML)
    ...
    def test_media_profile_follows_agent_profile_in_library(self):
        ...
        self.assertIn("const AGENT_STREAM_REFRESH_MS = 8 * 1000", app.HTML)
        ...
        self.assertIn('fetch(`/api/media-library?_=${now}`, { cache: "no-store" })', app.HTML)
        self.assertIn("const mediaChanged = JSON.stringify(mediaLibrary) !== JSON.stringify(nextMediaLibrary);", app.HTML)
        self.assertIn('document.addEventListener("visibilitychange"', app.HTML)
        ...
        self.assertIn("const AGENT_SLOT_COUNT = 10", app.HTML)
        self.assertIn("refs.agentNodeCount.textContent", app.HTML)
        self.assertIn("refs.hubNodeCount.textContent", app.HTML)
        self.assertNotIn("renderAgentSlot", app.HTML)
        self.assertNotIn("node-slot-placeholder", app.HTML)
        ...
        self.assertIn('id="nodeSpaceToggle"', app.HTML)
        self.assertIn('id="nodeSpacePanel" hidden', app.HTML)
        ...
        self.assertIn(".dashboard-grid.fixed-dashboard-grid > .dashboard-left-column { grid-column: 1 / span 6; }", app.HTML)
        self.assertIn(".dashboard-grid.fixed-dashboard-grid > .dashboard-right-column { grid-column: 7 / span 6; }", app.HTML)
        self.assertIn(".dashboard-grid.fixed-dashboard-grid > .dashboard-column {", app.HTML)
        self.assertIn('class="dashboard-column dashboard-left-column"', app.HTML)
        self.assertIn('class="dashboard-column dashboard-right-column"', app.HTML)
        self.assertIn("Agent 控制模块", app.HTML)
        self.assertIn('class="monitor-panel monitor-disclosure autotune-history-panel" data-monitor-section="autotune"', app.HTML)
        self.assertIn("function monitorDisclosureOpen", app.HTML)
        self.assertIn("rememberMonitorDisclosure", app.HTML)
        self.assertIn("function initializeDashboardLayout", app.HTML)
        self.assertIn('class="dashboard-grid fixed-dashboard-grid"', app.HTML)
        self.assertNotIn("fixed-content-grid", app.HTML)
        self.assertIn("fixed-layout-ready", app.HTML)
        self.assertNotIn("function beginDashboardDrag", app.HTML)
        self.assertNotIn("function beginDashboardResize", app.HTML)
        self.assertNotIn("DASHBOARD_MODULES", app.HTML)
        self.assertNotIn("dashboard-drag-ghost", app.HTML)
        self.assertNotIn("dashboard-module-handle", app.HTML)
        self.assertNotIn("dashboard-module-resize", app.HTML)
        self.assertIn("grid-template-columns: repeat(12, minmax(0, 1fr))", app.HTML)
        self.assertIn('.dashboard-grid.fixed-dashboard-grid > .command-strip { grid-column: 1 / -1; }', app.HTML)
        self.assertIn('<details class="card command-strip" id="commandStrip">', app.HTML)
        self.assertIn('<summary class="command-head command-summary">', app.HTML)
        ...
        self.assertNotIn('id="dashboardEditBtn"', app.HTML)
        self.assertNotIn('id="dashboardResetBtn"', app.HTML)
        monitor_position = app.HTML.index('<div class="card monitor-card">')
        resource_position = app.HTML.index('<div class="card resource-card">')
        node_table_position = app.HTML.index('<div class="card node-table-card agent-control-panel">')
        node_space_position = app.HTML.index('id="nodeSpaceToggle"')
        upload_position = app.HTML.index('<details class="card upload-card">')
        left_column_position = app.HTML.index('class="dashboard-column dashboard-left-column"')
        right_column_position = app.HTML.index('class="dashboard-column dashboard-right-column"')
        ...
        log_position = app.HTML.index('<details class="top-log-panel log-card">')
        command_position = app.HTML.index('<details class="card command-strip" id="commandStrip">')
        agent_control_position = app.HTML.index('<div class="card node-table-card agent-control-panel">')
        ...
        self.assertLess(command_position, left_column_position)
        self.assertLess(left_column_position, monitor_position)
        self.assertLess(monitor_position, resource_position)
        self.assertLess(resource_position, right_column_position)
        self.assertLess(right_column_position, agent_control_position)
        self.assertLess(agent_control_position, upload_position)
        self.assertLess(node_table_position, node_space_position)
        self.assertLess(node_space_position, upload_position)
        self.assertIn('${donut("CPU"', app.HTML)
        self.assertIn('${donut("内存"', app.HTML)
        self.assertIn('${donut("硬盘"', app.HTML)
        self.assertIn('${donut("推流"', app.HTML)
