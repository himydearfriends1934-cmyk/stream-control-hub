...
class AgentUpgradeTests(unittest.TestCase):
    ...
    def test_media_profile_follows_agent_profile_in_library(self):
        ...
        self.assertIn("const DASHBOARD_MODULES =", app.HTML)
        self.assertIn("function initializeDashboardLayout", app.HTML)
        self.assertIn("function beginDashboardDrag", app.HTML)
        self.assertIn("function beginDashboardResize", app.HTML)
        self.assertIn('className = "dashboard-drag-ghost"', app.HTML)
        self.assertIn("grid-template-columns: repeat(12, minmax(0, 1fr))", app.HTML)
        self.assertIn('const DASHBOARD_LAYOUT_STORAGE_KEY = "streamHub.dashboardLayout.v4";', app.HTML)
        ...
        self.assertIn('id="dashboardResetBtn"', app.HTML)
